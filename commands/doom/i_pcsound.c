...
static boolean CachePCSLump(sfxinfo_t *sfxinfo)
{
    ...
    while ((freq = PCSGetFreq())) {
	    beep(freq, (1000 * 1000) / 140);
    }
...
static int I_PCS_GetSfxLumpNum(sfxinfo_t* sfx)
{
    ...
    if (use_sfx_prefix)
    {
        M_snprintf(namebuf, (unsigned)sizeof(namebuf), "dp%s", DEH_String(sfx->name));
    }
    else
    {
        M_StringCopy(namebuf, DEH_String(sfx->name), (unsigned)sizeof(namebuf));
    }
